jQuery(function($, doc) {
	...
	var View = {
		init : function() {
			...
			self.bind('click', this.openModalCallback);
			self.trigger('initialize');
		},
		domTxtSet: function(title, fkanji, lkanji, fkana, lkana, birth, gcheck) {
			$(".modal h2").text(title);
		...
		openModalCallback: function() {
			...
			cld = $('#' + currentKanteiId);
			this.dataSet(currentKanteiId, currentKanteiClass);
			this.dspSet(cld);
			this.openPopup();
			return false;
		}
